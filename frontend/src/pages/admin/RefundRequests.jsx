...
const FILTER_STATUS_OPTIONS = ['Refund Requested', 'Pending', 'Refund Requested or Pending', 'All'];
...
const RefundRequests = () => {
  ...
  const [statusFilter, setStatusFilter] = useState('Refund Requested');
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      if (statusFilter === 'Refund Requested' && status !== 'refund requested') return false;
      if (statusFilter === 'Pending' && status !== 'pending') return false;
      if (statusFilter === 'Refund Requested or Pending' && status !== 'refund requested' && status !== 'pending') return false;
      ...
      <div className="mb-4">
        ...
        <div className="mt-3 flex items-center justify-between gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <button
            ...
            onClick={() => {
              setSearch('');
              setStatusFilter('Refund Requested');
              setCityFilter('All');
              setFromDate('');
              setToDate('');
            }}
